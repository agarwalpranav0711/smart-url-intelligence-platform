 ...
 */
async function createShortLink(targetUrl, userId, alias = null, expiresAt = null) {
  // 1. Custom Alias Path (No random code generation or retry loop)
  if (alias) {
    try {
      const link = await linksDb.createLink(alias, targetUrl, userId, expiresAt);
      return link;
    } catch (err) {
  ...
  while (attempts < MAX_INSERT_ATTEMPTS) {
    ...
    try {
      const link = await linksDb.createLink(shortCode, targetUrl, userId, expiresAt);
      return link;
    } catch (err) {
      if (err.code === PG_UNIQUE_VIOLATION_CODE) {
